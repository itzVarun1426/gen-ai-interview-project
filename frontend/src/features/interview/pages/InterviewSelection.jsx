          ...
          {options.map((option) => (
            ...
            >
              <div className={`p-2 rounded-lg transition-colors ${
                selectedOption === option.id ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-400'
              }`}>
                <option.icon size={20} />
              </div>
              <div className="text-left flex-1">
                <p className={`text-sm font-semibold ${
                  selectedOption === option.id ? 'text-blue-400' : 'text-slate-300'
                }`}>
                  {option.title}
                </p>
              </div>
          ...
          {options.map((option) => option.id === selectedOption && (
            ...
            >
              ...
              <button
                ...
                className={`group relative flex items-center justify-center gap-3 w-full py-5 rounded-2xl bg-gradient-to-r ${option.color} text-white font-bold text-lg shadow-xl hover:shadow-2xl transition-all active:scale-[0.98] overflow-hidden`}
              >
